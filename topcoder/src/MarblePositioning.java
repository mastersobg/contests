...
public class MarblePositioning {
    ...
    double rec(int idx, int []R, int []r, boolean []used) {
        if(idx == R.length) {
            return calc(r);
        }
        double ret = 1e+20;
        for(int i = 0; i < R.length; ++i)
            if(!used[i]) {
                used[i] = true;
                r[idx] = R[i];
                ret = min(ret, rec(idx + 1, R, r, used));
                used[i] = false;
            }
        return ret;
    }
    ...
    double calc(int []rad) {
        dbg("rad=", Arrays.toString(rad));
        double []pos = new double[rad.length];
        pos[0] = 0;
...
/*            eq(0,(new MarblePositioning()).totalWidth(new int[] {1, 2}),2.8284271247461903);
  ...
  */          eq(4,(new MarblePositioning()).totalWidth(new int[] {1,1,999950884}),63246.0);
        } catch( Exception exx) {
