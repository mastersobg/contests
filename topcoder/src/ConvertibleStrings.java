...
public class ConvertibleStrings {
    ...
    int rec(int letter) {
      ...
      int ret = next[letter].size() > 0 ? Integer.MAX_VALUE : 0;
      for (int a : next[letter]) {
        ret = min(ret, count(letter, a));
      }
      return ret + rec(letter + 1);
    }

    int count(int a, int b) {
      int ret = 0;
      for (int i = 0; i < v1.length; ++i) {
        if ((v1[i] == a && v2[i] != b) || (v2[i] == b && v1[i] != a)) {
          ++ret;
        }
      }
      return ret;
...
// BEGIN CUT HERE
  ...
  String arrayToString(Object o) {
    ...
    public static void main(String[] args) {
        try {
            DEBUG = true;
            eq(0,(new ConvertibleStrings()).leastRemovals("FHA", "III"),2);
